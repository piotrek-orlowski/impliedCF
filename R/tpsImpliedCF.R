...
tpsImpliedCF <- function(option.panels, mkt.frame, u.t.mat, discounted = TRUE, smoothing.results = NULL, doPlot=FALSE, doFitPlot = 0, verbose=FALSE, ...) {
  ...
  # }
    try({u.t.out[nn,3] <- cos(u*(r-q)*t) + 1i*sin(u*(r-q)*t);
      u.t.out[nn,3] <- u.t.out[nn,3] + 
        hcubature(function(k) otmFun(tps.fit,k=k,r=r,q=q,dT=t) * (-u^2*cos(u*k)+u*sin(u*k))/exp(k) * exp(r*t),lowerLimit = logF,upperLimit = U, tol = 1e-4, vectorInterface = TRUE)$integral + 
        1i*hcubature(function(k) otmFun(tps.fit,k=k,r=r,q=q,dT=t) * (-u^2*sin(u*k)-u*cos(u*k))/exp(k) * exp(r*t),lowerLimit = logF,upperLimit = U,tol = 1e-4, vectorInterface = TRUE)$integral
      u.t.out[nn,3] <- u.t.out[nn,3] + 
        hcubature(function(k) otmFun(tps.fit,k=k,r=r,q=q,dT=t) * (-u^2*cos(u*k)+u*sin(u*k))/exp(k) * exp(r*t),lowerLimit = L,upperLimit = logF, tol = 1e-4, vectorInterface = TRUE)$integral + 
        1i*hcubature(function(k) otmFun(tps.fit,k=k,r=r,q=q,dT=t) * (-u^2*sin(u*k)-u*cos(u*k))/exp(k) * exp(r*t),lowerLimit = L,upperLimit = logF, tol = 1e-4, vectorInterface = TRUE)$integral})
